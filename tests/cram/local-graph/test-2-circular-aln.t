...
Another synthetic circular test case.
  $ ${BIN_DIR}/raptor --out-fmt m4 -r ${PROJECT_DIR}/test-data/graph-mapping/synth-1-rotation/ecoli-0-100000.fasta -g ${PROJECT_DIR}/test-data/graph-mapping/synth-1-rotation/graph.gfa2 -d ${PROJECT_DIR}/test-data/graph-mapping/synth-1-rotation/read-1-12kbp_rotated-fwd.fasta -v 0 --align
...
Yet another synthetic circular test case.
  $ ${BIN_DIR}/raptor --out-fmt m4 -r ${PROJECT_DIR}/test-data/graph-mapping/synth-1-rotation/ecoli-0-100000.fasta -g ${PROJECT_DIR}/test-data/graph-mapping/synth-1-rotation/graph.gfa2 -d ${PROJECT_DIR}/test-data/graph-mapping/synth-1-rotation/read-2-12kbp_rotated-rev.fasta -v 0 --align
...
Graph alignment of the reference sequences on a small plasmid contig set. Here, contigs are used as the target, and the contig graph as the input graph. Reference sequences are used as queries.
  $ ${BIN_DIR}/raptor --out-fmt m4 -r ${PROJECT_DIR}/test-data/graph-mapping/real-1-plasmid/asm.plasmids.fa -g ${PROJECT_DIR}/test-data/graph-mapping/real-1-plasmid/contig.s2.gfa2 -d ${PROJECT_DIR}/test-data/graph-mapping/real-1-plasmid/ref.plasmids.fa -v 0 --align
