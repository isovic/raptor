...
std::shared_ptr<AlignerBase> createAlignerKSW2Double(const raptor::AlignmentOptions &opt) {
    return std::shared_ptr<AlignerBase>(new AlignerKSW2Double(opt));
}
...
AlignerKSW2Double::AlignerKSW2Double(const raptor::AlignmentOptions &opt) : opt_(opt) {}

AlignerKSW2Double::~AlignerKSW2Double() {}

std::shared_ptr<raptor::AlignmentResult> AlignerKSW2Double::Global(const char *qseq, int64_t qlen,
    ...
    int8_t q2 = -opt_.p.w[1].open;
    int8_t e2 = -opt_.p.w[1].ext;

    KSW2GlobalAlnWrapper_(km, (const int8_t *)qseq, qlen, (const int8_t *)tseq, tlen, 5, &mat[0], q,
                          e, q2, e2, opt_.bandwidth, zdrop, opt_.end_bonus, flag, &ez);
...
std::shared_ptr<raptor::AlignmentResult> AlignerKSW2Double::Extend(const char *qseq, int64_t qlen,
                                                                   const char *tseq, int64_t tlen) {
    auto result = raptor::createAlignmentResult();
    ...
    KSW2GlobalAlnWrapper_(km, (const int8_t *)qseq, qlen, (const int8_t *)tseq, tlen, 5, &mat[0], q,
                          e, q2, e2, opt_.zbandwidth, opt_.zdrop, opt_.end_bonus, flag, &ez);
...
std::shared_ptr<raptor::AlignmentResult> AlignerKSW2Double::Local(const char *qseq, int64_t qlen,
                                                                  const char *tseq, int64_t tlen) {
    auto result = raptor::createAlignmentResult();
...
std::shared_ptr<raptor::AlignmentResult> AlignerKSW2Double::Semiglobal(const char *qseq,
                                                                       int64_t qlen,
                                                                       const char *tseq,
                                                                       int64_t tlen) {
    auto result = raptor::createAlignmentResult();
...
void AlignerKSW2Double::KSW2GlobalAlnWrapper_(void *km, const int8_t *qseq_, int qlen,
                                              const int8_t *tseq_, int tlen, int8_t m,
                                              const int8_t *mat, int8_t q, int8_t e, int8_t q2,
    ...
    ksw_extd2_sse(km, qlen, (const uint8_t *)&qseq[0], tlen, (const uint8_t *)&tseq[0], m, mat, q,
