...
ArgumentParser::ArgumentParser() {
    //  AddArgument(NULL, VALUE_TYPE_NONE, "h", "help", "", "Displays this list of commands.", 0,
    //  "Help");
}
...
void ArgumentParser::AddArgument(void *target, ValueType value_type, std::string arg_short,
                                 std::string arg_long, std::string default_value,
                                 std::string description, int32_t positional,
                                 std::string argument_group) {
    if (arg_short == "" && arg_long == "") {
        fprintf(stderr,
                "Warning: Argument initialization. Neither a short nor a long name of argument "
                "specified. Omitting.\n\n");
        fflush(stderr);
        return;
    }
    ...
    /// In case the argument doesn't require a parameter (but is a switch instead), and the given
    /// value is not 0 or 1, set the default value to 0 (False).
    ...
    if (arg.positional != 0) valid_args_positional_[arg.positional] = (arguments_.size() - 1);
...
Argument *ArgumentParser::CheckArguments_(std::string arg_name) {
    ...
    if (arg_name[1] != '-') {
        ...
        if (it != valid_args_short_.end()) return &(arguments_[it->second]);
        return NULL;
        ...
        if (it != valid_args_long_.end()) return &(arguments_[it->second]);
        return NULL;
    }
...
void ArgumentParser::ProcessArguments(int argc, char *argv[], int offset) {
    ...
    for (int32_t i = offset; i < argc; i++) {
        ...
        if (arg[0] == '-' && arg.size() > 1) {
            ...
            if (arg.size() == 1) {
                ...
                // If the second character is not '-' then the argument is specified by its short
                // name.
                ...
                if (argument_index >= 0) {
                    ...
                    if (arguments_[argument_index].value_type != VALUE_TYPE_NONE &&
                        arguments_[argument_index].value_type !=
                            VALUE_TYPE_BOOL) {  // This argument expects a value.
                        if ((i + 1) < argc) {
                            arguments_[argument_index].value = argv[i + 1];
                            SetArgumentTarget_(arguments_[argument_index].target,
                                               arguments_[argument_index].value_type,
                                               arguments_[argument_index].value);
                            i += 1;
                        } else {
                        ...
                        /// Handle the special case when the argument is a composite one. Need to
                        /// find the composite definition and expand it (recursively).
                        if (arguments_[argument_index].value_type == VALUE_TYPE_COMPOSITE) {
                            ...
                            if (it_arg == composite_args_.end()) {
                                fprintf(stderr,
                                        "ERROR: Composite parameter '%s' not defined. Exiting.\n",
                        ...
                        if (arguments_[argument_index].value == "0" ||
                            arguments_[argument_index].value == "false") {
            ...
            if (valid_args_positional_.find(position_back) != valid_args_positional_.end()) {
                ...
                SetArgumentTarget_(arguments_[valid_args_positional_[position_back]].target,
                                   arguments_[valid_args_positional_[position_back]].value_type,
                                   arguments_[valid_args_positional_[position_back]].value);

            } else if (valid_args_positional_.find(position_front) !=
                       valid_args_positional_.end()) {
                arguments_[valid_args_positional_[position_front]].value = arg;
                arguments_[valid_args_positional_[position_front]].is_set = true;
                SetArgumentTarget_(arguments_[valid_args_positional_[position_front]].target,
                                   arguments_[valid_args_positional_[position_front]].value_type,
                                   arguments_[valid_args_positional_[position_front]].value);
    ...
    /// Check if there are any positional arguments that were not loaded. These are necessary
    /// arguments.
    for (uint32_t i = 0; i < arguments_.size(); i++) {
        if (arguments_[i].positional != 0 && arguments_[i].is_set == false) {
            std::string argument_name =
                (arguments_[i].arg_long != "") ? arguments_[i].arg_long : arguments_[i].arg_short;
            fprintf(stderr,
                    "ERROR: Not all required arguments specified. Parameter '%s' missing a value. "
                    "Exiting.\n",
                    argument_name.c_str());
            exit(1);
        }
...
std::string ArgumentParser::VerboseUsage() {
    ...
    const int32_t description_starting_char =
        value_type_starting_char + 3 + 3;  /// 3 is the length of the type name, and 3 is the
                                           /// spacing between the type name and the description.
    ...
    if (program_name_.size() > 0) {
        ...
        for (int32_t i = 0; i < arguments_.size(); i++) {
            if (arguments_[i].positional == 0) num_non_positional_args += 1;
        }
        ...
        std::sort(positional_arguments_back.begin(), positional_arguments_back.end(),
                  positional_less_than_key());
        std::sort(positional_arguments_front.begin(), positional_arguments_front.end(),
                  positional_less_than_key());

        for (uint32_t i = 0; i < positional_arguments_front.size(); i++) {
            std::string arg_name = positional_arguments_front[i].arg_long;
            if (arg_name == "") arg_name = positional_arguments_front[i].arg_short;
        ...
        for (uint32_t i = 0; i < positional_arguments_back.size(); i++) {
            ...
            if (arg_name == "") arg_name = positional_arguments_back[i].arg_short;
            ret_ss << " " << arg_name.c_str();
        }
        ret_ss << "\n\n";
    }
    ...
    for (uint32_t group_id = 0; group_id < arg_groups_in_order_of_appearance_.size(); group_id++) {
        ...
        for (uint32_t i = 0; i < it->second.size(); i++) {
            ...
            if (arguments_[it->second.at(i)].positional == 0) {
                ...
                if (arguments_[it->second.at(i)].arg_short != "" &&
                    arguments_[it->second.at(i)].arg_long != "") {
                    ss << ", ";
                    num_chars += 2;
                } else {
                    ss << "  ";
                ...
                if (arg_name == "") arg_name = arguments_[it->second.at(i)].arg_short;
            ...
            int32_t num_empty_chars = (((value_type_starting_char - num_chars) > 0)
                                           ? (value_type_starting_char - num_chars)
                                           : 1);
            ...
            //      else {
            //        ss << arguments_[it->second.at(i)].value_type << std::string((3 -
            //        arguments_[it->second.at(i)].value_type.size()), ' ');
            //      }
            ...
            if (arguments_[it->second.at(i)].description != "") {
                int32_t num_empty_chars = (((description_starting_char - num_chars) > 0)
                                               ? (description_starting_char - num_chars)
                                               : 1);
                std::string empty_chars(num_empty_chars, ' ');
                ss << empty_chars;
                ...
                ss << WrapString_(description_starting_char, wrap_width,
                                  arguments_[it->second.at(i)].description);
                num_chars += num_empty_chars + arguments_[it->second.at(i)].description.size();
            }
            if (arguments_[it->second.at(i)].value_type != VALUE_TYPE_NONE &&
                arguments_[it->second.at(i)].default_value != "") {
                if (arguments_[it->second.at(i)].value_type != VALUE_TYPE_BOOL) {
                    ss << " [" << arguments_[it->second.at(i)].default_value << "]";
                } else {
                    ss << " ["
                       << ((arguments_[it->second.at(i)].default_value == "0" ||
                            arguments_[it->second.at(i)].default_value == "false")
                               ? "false"
                               : "true")
                       << "]";
                }
...
std::string ArgumentParser::VerboseArguments() {
    ...
    for (uint32_t i = 0; i < arguments_.size(); i++) {
        ss << "'-" << arguments_[i].arg_short << "'"
           << std::string(1 + padding_string.size() - arguments_[i].arg_short.size(), ' ');
        ss << "'--" << arguments_[i].arg_long << "'"
           << std::string(max_arg_len + padding_string.size() - arguments_[i].arg_long.size(), ' ');
...
Argument *ArgumentParser::GetArgument(std::string arg_name) {
    ...
    if (it == valid_args_all_.end()) return NULL;
...
Argument *ArgumentParser::GetArgumentByShortName(std::string arg_name) {
    ...
    if (it == valid_args_short_.end()) return NULL;
    return (&(arguments_.at(it->second)));
}
...
Argument *ArgumentParser::GetArgumentByLongName(std::string arg_name) {
    ...
    if (it == valid_args_long_.end()) return NULL;
    return (&(arguments_.at(it->second)));
}
...
int ArgumentParser::GetValue(std::string arg_name, int64_t *value) {
    std::map<std::string, int32_t>::iterator it = valid_args_all_.find(arg_name);
    if (it == valid_args_all_.end()) return 1;
    ValueType value_type = arguments_.at(it->second).value_type;
    //  if (value_type != VALUE_TYPE_INT32 || value_type != VALUE_TYPE_INT64 || value_type !=
    //  VALUE_TYPE_UINT32 || value_type != VALUE_TYPE_UINT64)
    if (value_type != VALUE_TYPE_INT64) return 1;
    *value = std::stol(arguments_.at(it->second).value);
    return 0;
}
...
int ArgumentParser::GetValue(std::string arg_name, float *value) {
    std::map<std::string, int32_t>::iterator it = valid_args_all_.find(arg_name);
    if (it == valid_args_all_.end()) return 1;
    if (arguments_.at(it->second).value_type != VALUE_TYPE_FLOAT) return 1;
    *value = std::stof(arguments_.at(it->second).value);
    return 0;
}
...
int ArgumentParser::GetValue(std::string arg_name, std::string *value) {
    std::map<std::string, int32_t>::iterator it = valid_args_all_.find(arg_name);
    if (it == valid_args_all_.end()) return 1;
    if (arguments_.at(it->second).value_type != VALUE_TYPE_STRING) return 1;
    *value = arguments_.at(it->second).value;
    return 0;
}

void ArgumentParser::SetArgumentTarget_(void *target, ValueType value_type,
                                        std::string &argument_value) {
    if (value_type == VALUE_TYPE_NONE) return;
...
const std::string &ArgumentParser::get_program_name() const { return program_name_; }
